...
||`Timezone`| Time zone for `InputStart` and `InputEnd` (will override the timezone without shifting the time) and time series values that are output. The Campbell Cloud UTC times will be converted to the requested time zone.  See the [Aquarius Datastore Timezone](../datastore-ref/Aquarius/Aquarius.md#timezone-handling) documentation. | Output will use the local time zone for the computer. |
