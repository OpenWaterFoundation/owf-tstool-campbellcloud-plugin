...
This chapter provides an overview of getting started with TSTool and the Campbell Cloud plugin commands.
...
The remainder of this documentation focuses on getting started with TSTool using the Campbell Cloud plugin.
...
## Installing the Campbell Cloud Plugin ##

See the [Appendix - Install Plugin](../appendix-install/install.md) documentation for instructions to install the plugin.

## Listing Campbell Cloud Time Series ##

The main TSTool user interface can be used to list Campbell Cloud time series, as follows:

1.  Select a Campbell Cloud datastore from the ***Datastore*** choices.
...
TSTool Campbell Cloud plugin commands are provided to read time series from Campbell Cloud.
Additional commands may be added in the future as needed.
The primary uses of the plugin commands are:

*   read time series from Campbell Cloud web services to allow processing by other TSTool commands
*   automate visualizing Campbell Cloud time series
*   combine time series from Campbell Cloud and other sources

The following is a summary of plugin commands,
which are listed in the ***Commmands(Plugin)*** menu.

**<p style="text-align: center;">
Campbell Cloud Plugin Commands
</p>**

| **Command** | **Description** |
| -- | -- |
| [`ReadCampbellCloud`](../command-ref/ReadCampbellCloud/ReadCampbellCloud.md) | Read one or more time series from the Campbell Cloud web services. |

See also the [`TSID`](../command-ref/TSID/TSID.md) command format for Campbell Cloud time series.
