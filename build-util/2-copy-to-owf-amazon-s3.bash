...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Copy the TSTool Campbell Cloud plugin installer files to the Amazon S3 static website folder:"
...
uploadInstaller() {
  ...
  installerUploadCount=0
    ...
    if [ "$includeWindows" = "yes" ]; then
      ...
      zipFilePattern="${distFolder}/tstool-campbellcloud-plugin-${pluginVersion}-win-*.zip"
      ...
      if [ -f "${latestZipFile}" ]; then
        ...
        if [ ${errorCode} -ne 0 ]; then
          ...
          cloudFrontFile="/tstool-campbellcloud-plugin/${pluginVersion}/software/*"
...
logInfo ""
logInfo "${installerUpdateCount} TSTool Campbell Cloud plugin installers were uploaded to Amazon S3 location:"
logInfo ""
logInfo "  ${s3FolderUrl}"
logInfo "  ${gpDownloadUrl}"
logInfo ""
logInfo "Next steps are to do the following:"
logInfo "  - Visit the following folder to download the TSTool Campbell Cloud plugin:"
logInfo "      ${gpDownloadUrl}/"
logInfo ""
